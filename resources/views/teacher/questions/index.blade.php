...
<div class="page-container">
    ...
    {{-- Separate MCQs --}}
    @php
        $mcqs = $questions->where('question_type', 'mcq');
        $shorts = $questions->where('question_type', 'short');
    @endphp

    @if($mcqs->count() > 0)
        <h3 class="font-semibold mb-2">MCQ Questions</h3>
        <table class="min-w-full bg-white border rounded shadow overflow-hidden mb-6">
            ...
            <tbody>
                @foreach($mcqs as $question)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 border text-center">{{ $question->marks_mcq }}</td>
    ...
    @if($shorts->count() > 0)
        <h3 class="font-semibold mb-2">Short Answer Questions</h3>
