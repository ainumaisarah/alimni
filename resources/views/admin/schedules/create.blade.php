...
<div class="page-container">
    <div class="flex items-center gap-2 mb-6">
        <a href="{{ route('admin.schedules.index') }}" class="h-8 w-8 inline-flex items-center justify-center p-2 bg-gray-100 hover:bg-gray-200 rounded-lg">←</a>
                ...
                <select name="classroom_id" id="classroom_id" class="w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($classrooms as $classroom)
                        <option value="{{ $classroom->id }}"
                                ...
                                {{ (request('classroom_id') == $classroom->id || (isset($selectedClassroomId) && $selectedClassroomId == $classroom->id)) ? 'selected' : '' }}>
                            {{ $classroom->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="teacher_id" id="teacher_id" class="w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($teachers as $teacher)
                        <option value="{{ $teacher->id }}"
                            {{ (request('classroom_id') && $teacher->id == optional($classrooms->firstWhere('id', request('classroom_id')))->teacher_id) ? 'selected' : '' }}>
                            {{ $teacher->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <div class="schedule-row border rounded p-4 mb-4 relative">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="slot-count w-full border rounded px-3 py-2" required>
                            ...
                            <option value="1">1 Slot (30 minutes)</option>
                            <option value="2">2 Slots (60 minutes)</option>
                        </select>
                    </div>

                    <!-- Hidden actual values -->
                    <input type="hidden" name="schedules[0][start_time]" class="start-time">
                    <input type="hidden" name="schedules[0][end_time]" class="end-time">

                    <p class="text-sm text-gray-600 mt-2 result-text"></p>
                </div>
